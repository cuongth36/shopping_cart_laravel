...
class PostController extends Controller
{
    ...
    function show(){
        ...
        $product_seller = DB::select("SELECT SUM(ord.qty) as total_qty, prd.title,prd.id,prd.title,prd.slug,prd.thumbnail,prd.price ,ord.product_id FROM `order_details` as ord INNER JOIN products as prd on ord.product_id = prd.id GROUP BY ord.product_id ORDER BY total_qty desc limit 5 ");
    ...
    function detail($slug){
        ...
        if(count($db_select) > 0){
            ...
            $related_articles = Post::where('slug', '<>', $slug_new)->limit(3)->get();
            $product_seller = DB::select("SELECT SUM(ord.qty) as total_qty, prd.title,prd.id,prd.title,prd.slug,prd.thumbnail,prd.price ,ord.product_id FROM `order_details` as ord INNER JOIN products as prd on ord.product_id = prd.id GROUP BY ord.product_id ORDER BY total_qty desc limit 5 ");
