    ...
    function list(Request $request){
        ...
        if ($status == 'pendding'){
            $list_action = [
                ...
                $orders =  DB::select(
                    "SELECT b.price,SUM(b.qty) as qty,a.* from orders a inner join order_details b on a.id = b.order_id where a.status = '0' GROUP BY (a.id) LIMIT {$num_per_page} OFFSET {$offset} "
                );
                
        }
        else if($status == 'trash'){
                $orders =  DB::select(
                "SELECT b.price,SUM(b.qty) as qty,a.* from orders a inner join order_details b on a.id = b.order_id where a.status = '2' GROUP BY (a.id) LIMIT $num_per_page OFFSET {$offset}"
                );
        }else{
                $orders =  DB::select(
                "SELECT b.price,SUM(b.qty) as qty,a.* from orders a inner join order_details b on a.id = b.order_id where a.status = '1' GROUP BY (a.id) LIMIT $num_per_page OFFSET {$offset}"
                );  
        }
